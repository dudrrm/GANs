...
def weights_init(m):
    """Initialize the weights to each layer."""
...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm, use_dropout=False, gpu_ids=[]):
    ...
    if norm == 'batch':
        ...
        print('nomralization layer {} is not found'.format(norm))
    ...
    if which_model_netG == 'resnet_9blocks':
        ...
        print('Generator model name {} is not defined.'.format(which_model_netG))

    # set the model to the cuda memory.
    if len(gpu_ids) > 0:
        netG.cuda(gpu_ids[0])
...
def define_D(input_nc, ndf, which_model_netD, n_layers=3, use_sigmoid=False, gpu_ids=[]):
    """Define Discriminator."""
    netD = None #Initialize
    ...
    if which_model_netD == 'basic':
        ...
        netD = NLayerDiscriminator(input_nc, ndf, n_layers, use_sigmoid, gpu_ids=gpu_ids)
    else:
        print('Discriminator model name {} is not defined.'.format(which_model_netD))
...
###############################################################################
#                                 Classes                                     #
# The belows follow nn.Module.
# 1. Loss Function
# 2. Resnet Generator
# 3. Resnet Block
# 4. Unet Generator
# 5. Unet Skip Connection
# 6. N-Layer Discriminator
...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        """For the un-preprocessed target dataset, customize them as a tensor form."""
        ...
        if target_is_real:
            ...
            if create_label:
                # If the labels do not exist or not correct, create them into the tensor form.
                real_tensor = torch.ones(input.size()).to(device)  #self.Tensor(input.size()).fill_(self.real_label)
            ...
            if create_label:
                fake_tensor = torch.zeros(input.size()).to(device)  #self.Tensor(input.size()).fill(self.fake_label)
