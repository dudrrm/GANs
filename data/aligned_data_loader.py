...
class PairedData(object):
    ...
    def __next__(self):
        ...
        w_total = AB.size(3)
        w = int(w_total / 2)    # width
        h = AB.size(2)          # height

        w_offset = random.randint(0, max(0, w - self.fineSize - 1))
        h_offset = random.randint(0, max(0, h - self.fineSize - 1))
        A = AB[:, :, h_offset : h_offset + self.fineSize, w_offset : w_offset + self.fineSize]
        B = AB[:, :, h_offset : h_offset + self.fineSize, w + w_offset : w + w_offset + self.fineSize]
        # print('aligned data loader step ==> input real A size: {} \tinput real B size: {}'.format(A.size(), B.size()))
